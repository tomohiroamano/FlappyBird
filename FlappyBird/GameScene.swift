...
class GameScene: SKScene, SKPhysicsContactDelegate /* 追加 */  {
    ...
    var itemNode:SKNode! //課題：アイテム用のノードを追加
    ...
    let itemuserDefaults:UserDefaults = UserDefaults.standard    // 追加
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        // 課題：アイテムとぶつかった時の処理を追加
            ...
            // 効果音を鳴らす
            let mySoundAction: SKAction = SKAction.playSoundFileNamed("correct2.mp3", waitForCompletion: false)
            // 再生アクション
            run(mySoundAction)
            
            // 課題：bodyAがitemcategoryであればアイテムノードを削除する
            if  (contact.bodyA.categoryBitMask & itemCategory) == itemCategory {
                contact.bodyA.node?.removeFromParent()
            }
            // 課題：bodyBがitemcategoryであればアイテムノードを削除する
    ...
    func setupItemScoreLabel() {
        ...
        itemscoreLabelNode.zPosition = 110 // ★scoreLabelの前（一番前）
        ...
        bestitemScoreLabelNode.zPosition = 110 // scoreLableの前(一番手前に表示する)
        bestitemScoreLabelNode.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.left
        
        let bestitemScore = userDefaults.integer(forKey: "ITEMBEST") //★forkey?
    ...
    func restart() {
        ...
        //★ アイテムスコアをリセット
        itemscore = 0
        scoreLabelNode.text = String("ItemScore:\(itemscore)")
    ...
    func setupItem() {
        ...
        // 画面外まで移動するアクションを作成
        let moveItem = SKAction.moveBy(x: -movingDistance * 2, y: 0, duration:4)
        ...
        // ★アイテムの出現位置を規定するため、壁の画像サイズを取得
        let wallTexture = SKTexture(imageNamed: "wall")
        //let wallSize = SKTexture(imageNamed: "wall").size()
        
        // ★アイテム出現位置の上下の振れ幅を鳥のサイズの5倍とする
        let random_y_range = itemSize.height * 5
        
        // ★アイテム出現位置の左右の振れ幅を鳥のサイズの3倍とする
        let random_x_range = itemSize.height * 3
        ...
        let createItemAnimation = SKAction.run({
            ...
            // ★ -random_y_range〜random_y_rangeまでのランダム値を生成
            let random_y = CGFloat.random(in: -random_y_range..<random_y_range)
            // ★中央位置を基準にランダムな値（プラスマイナス）を足して、アイテムのY座標を決定
            let item_y = center_y + random_y
            
            // ★ random_xの作成
            let random_x = CGFloat.random(in: 0..<random_x_range)
            // ★ アイテムのX座標を決定　「壁の幅3個分隣」に「アイテム幅の5倍までのランダム値」を足す
            let item_x = wallTexture.size().width * 3 + random_x
            
            // ★ アイテムを作成　X座標もY座標も指定
            let citem = SKSpriteNode(texture: itemTexture)
            citem.position = CGPoint(x: item_x, y: item_y)
            
            // スプライトに物理演算を設定する
            citem.physicsBody = SKPhysicsBody(circleOfRadius: citem.size.height / 2)
            citem.physicsBody?.categoryBitMask = self.itemCategory    // ←追加
            citem.physicsBody?.contactTestBitMask = self.birdCategory
            
            // 衝突の時に動かないように設定する
            citem.physicsBody?.isDynamic = false
        ...
        // ★アイテムの発生間隔をランダム
        let itemterm = Double.random(in: 1..<6)
        // ★次のアイテム作成までの時間待ちのアクションを作成　壁の時間と合わせる　元は4
        let waitAnimation = SKAction.wait(forDuration: itemterm)
